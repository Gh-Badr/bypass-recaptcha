...
def calculate_accuracy():
    ...
    def parse_list(list_str, is_response=False):
        ...
        if is_response:
            # Handle response format: remove quotes and extra spaces, then convert to integers
            return set(int(x.strip().strip("'\"")) for x in list_str.split(',')) if list_str else set()
        else:
            # Handle correct answer format: convert to integers directly
    ...
    with open('reordered_human_responses.csv', mode='r') as file1, open('reordered_responses.csv', mode='r') as file2:
        for line1, line2 in zip(file1, file2):
            ...
            # Convert size to integer and calculate total choices
            size = int(size_str1)
